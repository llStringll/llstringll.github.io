...
// jQueryScript.setAttribute('src','https://rawgit.com/farzher/fuzzysort/master/fuzzysort.js');
...
const sspSbGen = (select) => {
    ...
    const  searchIn = (value) => {
        ...
        for (let i = 0; i < nodes.length; i++) {
            if (fuzzysort.single(value, nodes[i]) != null) {
                if (fuzzysort.single(value, nodes[i]).score < -50){
